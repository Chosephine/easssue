...
def url_to_summary(url):
    ...
    ## ex : ['가나', '다라마', '바사', '아자', '차카타파하', ...]
    nouns = []
    for ns in news_nouns:
        for n in ns.split():
            nouns.append(n)
    # print(nouns) 
    
    ### 기사 본문 내 명사의 출현 횟수 dict
    news_nouns_counter = Counter(nouns)
    top_news_nouns = dict(news_nouns_counter.most_common(25))
